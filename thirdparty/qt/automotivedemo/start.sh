...
export LD_LIBRARY_PATH=/opt/locationstudio/ncdb/libs:$LD_LIBRARY_PATH
#appman --force-single-process -r -c /opt/am/config.yaml -c am-config.yaml --dbus none MainWithCluster.qml

appman --force-multi-process -r -c /opt/am/config.yaml -c am-config.yaml MainWithCluster.qml
