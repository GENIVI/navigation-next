...
CORE_SERVICES_LIB_ROOT ?= $(PROJ_ROOT)/output/$(PLATFORM)/$(TARGET)
...
NBIPAL_LIB_ROOT ?= $(NBIPAL_ROOT)/output/$(PLATFORM)/$(TARGET)
...
# unit test links with arhive so that it can access internal symbols
EXTRA_OBJS += $(CORE_SERVICES_LIB_ROOT)/libcoreservices.a
ifneq ($(DEBUG),0)
EXTRA_OBJS += $(NBIPAL_LIB_ROOT)/libnbpal.a
LDFLAGS += -lstdc++
else
LDFLAGS += -L$(NBIPAL_LIB_ROOT) -lnbpal
endif
...
test:
	...
	$(Q)LD_LIBRARY_PATH=$(CORE_SERVICES_LIB_ROOT):$(NBIPAL_LIB_ROOT):$(LD_LIBRARY_PATH) $(OBJ_ROOT)/unittests

.PHONY: memcheck_test
memcheck_test:
	@echo Running $(PROJ_NAME) unittest with Valgrind
	$(Q)LD_LIBRARY_PATH=$(CORE_SERVICES_LIB_ROOT):$(NBIPAL_LIB_ROOT):$(LD_LIBRARY_PATH) valgrind -v --leak-check=full --show-reachable=yes $(OBJ_ROOT)/unittests
