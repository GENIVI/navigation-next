...
//
// LBS Driver API version number
//
#define LBSDRIVERAPI_MAJ_VER   3
#define LBSDRIVERAPI_MIN_VER   1
#define LBSDRIVERAPI_REV_VER   0

#define MIN_LBSDRIVERAPI_VERSION(maj, min)\
    (LBSDRIVERAPI_MAJ_VER > maj || (LBSDRIVERAPI_MAJ_VER == maj && LBSDRIVERAPI_MIN_VER >= min))
...
typedef enum {
    ...
    GPS_QCOM_FIX_MODE_SPEED_OPTIMAL = 257,
    GPS_QCOM_FIX_MODE_ACCURACY_OPTIMAL = 258,
    GPS_QCOM_FIX_MODE_DATA_OPTIMAL = 259
} GPS_MODE;
...
// The following GPS_VALID fields is only applicable for a GPS_POSITION V2 data structure.
...
#define GPS_ERR_ROAMING_NOT_SUPPORTED                 0xe00120b1  // Unsupported roaming network
#define GPS_ERR_UPDATE_REQUIRED                       0xe00120c0  // LBSDriver update is required

// Service authorization errors
#define GPS_ERR_FAILED_SERVICE_AUTHORIZATION          0xe00120d0  // Service authorization error
#define GPS_ERR_PENDING_SERVICE_AUTHORIZATION         0xe00120d1  // Service authorization pending

// 
// GPS_INIT contains information to initialize LBS Driver
//
...
//
// GPS_POSITION_ERROR data structure contains additional error information
// associated with a position fix.
//
...
//
// GPS_FIX_RATE contains information to setup GPS fix frequency
//
...
//
// GPS_QOS contains information to specify the quality of the position fix
//
...
//
// This structure contains the additional status information about the GPS hardware
//
...
typedef struct _GPS_POSITION {
    DWORD dwVersion;             // Version of the data structure. 
    ...
    // Data introduced in version 2.
    GPS_POSITION_ERROR PositionUncertaintyError;
    GPS_MODE FixMode;
    DWORD dwFixError;
} GPS_POSITION, *PGPS_POSITION;

// 
// GPS_DEVICE contains information about the device driver and LBS Driver.
// It is returned by a call to GPSGetDeviceState().
// 
typedef struct _GPS_DEVICE {
    DWORD    dwVersion;                                 // Version of the data structure
    DWORD    dwSize;                                    // sizeof this structure
    DWORD    dwServiceState;                            // State of the LBS Driver. (See GPS_LBSDRIVER_STATE_XXX)
...
//          the LBS driver.
//
//          If unsuccessful, returns NULL. Call ::GetLastError() to find error code.
//
// Remarks:
...
///////////////////////////////////////////////////////////////////////////////
LBSDRIVER_API 
HANDLE GPSOpenDevice(HANDLE       hNewLocationData,    // CE event
...
//                                                          assigned by VZW).
// 
//                     GPS_QUERY_FIX        Starts a GPS query fix.  When a GPS fix is
...
//                                          pbParamData     NULL
//                                                          
//                     GPS_START_FIX        Starts a GPS tracking fix.  When a GPS fix is
//                                          available, the application is sent a
//                                          signal to the event that was specified
//                                          as the hNewLocationData parameter of
//                                          the GPSOpenDevice function.
...
//                                                          field set to zero.
// 
//                     GPS_STOP_FIX         Stops a GPS operation.
// 
//                                          pbParamData     NULL
// 
...
///////////////////////////////////////////////////////////////////////////////
LBSDRIVER_API 
DWORD GPSSetDeviceParam(HANDLE hGPSDevice,
			DWORD  dwParamName,
			BYTE*  pbParamData,
			DWORD  dwDataLen);
...
///////////////////////////////////////////////////////////////////////////////
LBSDRIVER_API 
DWORD GPSGetDeviceParam(HANDLE hGPSDevice,
			DWORD  dwParamName,
			BYTE*  pbParamData,
			DWORD* pdwDataLen);
...
//          the current state of the LBS Driver and GPS hardware.
//
// Parameters:
//      hGpsDevice      Handle returned by a call to GPSOpenDevice.
//
//      pGPSDeviceState Pointer to a data buffer of type GPS_DEVICE.
//                      Application should set the dwVersion field to
//                      GPS_VERSION_2 and dwSize before calling GPSGetDeviceState().
//
// Return values:
//     If successful, this function returns ERROR_SUCESS.
//     If unsuccessful, returns an error.
...
//
///////////////////////////////////////////////////////////////////////////////
LBSDRIVER_API DWORD GPSGetDeviceState (HANDLE      hGPSDevice,
				       GPS_DEVICE* pGPSDeviceState);
...
//      hGpsDevice      Handle returned by a call to GPSOpenDevice.
//
//      pGPSPosition    Pointer to a data buffer of type GPS_POSITION. 
//                      Application should set the dwVersion field to
//                      GPS_VERSION_2 and the dwSize before calling GPSGetPosition().
//
...
//     with a dwParamName value of GPS_QUERY_FIX or GPS_START_FIX.
//
///////////////////////////////////////////////////////////////////////////////
LBSDRIVER_API 
DWORD GPSGetPosition (HANDLE        hGPSDevice,
