...
CORETECH_ROOT    := $(realpath $(shell echo $(PROJ_ROOT) | sed 's/\/$(PROJ_NAME).*$$//g')/../../..)

# find branch
BRANCH ?= $(notdir $(realpath $(PROJ_ROOT)/../..))
...
ifeq ($(NIMCORE_SDK_INC),)
$(error environmental NIMCORE_SDK_INC variable is not defined.)
endif

ifeq ($(NIMCORE_SDK_BIN),)
$(error environmental NIMCORE_SDK_BIN variable is not defined.)
endif
...
EXCLUDE_SRCS :=$(PROJ_ROOT)/src/testgpssm.c

# where header files are located.
INCLUDE_DIRS := $(PROJ_ROOT)/include
INCLUDE_DIRS += $(PROJ_ROOT)/include/linux
INCLUDE_DIRS += $(NIMCORE_SDK_INC)
INCLUDE_DIRS += $(NIMCORE_SDK_INC)/linux
