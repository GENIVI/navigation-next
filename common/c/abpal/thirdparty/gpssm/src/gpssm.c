...
{
	int	smInitialized;	/*  security Module Initalized? 0: no, 1: yes */
};
        ...
        ABPAL_GpsLocation* result)
{
	...
	if (wgtochina_lb(securityModule->smInitialized, wgsLon, wgsLat, wgsHeight,
	...
	*result = *gpsLocation;
	...
	securityModule->smInitialized = 1;
	return PAL_Ok;
}
