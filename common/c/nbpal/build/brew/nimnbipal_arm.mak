...
!INCLUDE nimnbipal_common.mak

EXETYPE         = elf	   # Target image file ext
...
CFLAGS = $(CFLAGS0) $(CFLAGS1) -DAPP_CLSID=$(GEN_APP_CLS_ID)
...
!INCLUDE nimnbipal_common_files.mak          # All files

#----------------------------------------------------------------------------
# Lib file targets
#----------------------------------------------------------------------------

nimnbipal.$(OBJ) : $(NIMNBIPAL)
        @cmd /c echo ---------------------------------------------------------------
        @cmd /c echo TARGET $@
        $(LD) --partial $(LINK_CMD) nimnbipal.$(OBJ) $(NIMNBIPAL)

#$(TARGET).$(MODULE) : $(TARGET).$(EXETYPE) $(NIMNBIPAL)
#        @cmd /c echo ---------------------------------------------------------------
#        @cmd /c echo TARGET $@
#        $(HEXTOOL)  $(TARGET).$(EXETYPE) $(BINFORMAT) $(TARGET).$(MODULE)

#$(TARGET).$(EXETYPE) : $(ALL_APP_OBJS) $(NIMNBIPAL)
#        @cmd /c echo ---------------------------------------------------------------
#        @cmd /c echo TARGET $@        
#        $(LD) $(LINK_CMD) $(TARGET).$(EXETYPE) $(LFLAGS) $(ALL_APP_OBJS) nimnbpal.o $(LINK_ORDER)        
